...
arrestData <- read.csv(("ArrestDat.csv"), header=TRUE)
precincts1 <- sf::st_read("precincts1/nypp.shp")
map_data = arrestData[arrestData$Year == 2017, ]

#exp_minus_one <- function(x) { round( exp(x)-1 ) }
fixed_logarithmatic <- function(x) { x }
  ...
  fluidPage(fluidRow(
    ...
    column(4,      
           box(title="Map Options", status = "primary", solidHeader=T, collapsible=T, width = '100%',
               selectInput('year', 'Year', seq(2005, 2020), selectize=TRUE),
    ...
    linearValues <- {
      if(input$colorby == "race_dist"){
        ...
        switch(as.character(input$race),
               ...
               "O" = filteredData()$HispA/filteredData()$Other*1000,
        )
      } else {
